...
export class RevoluteJoint extends Joint {
  // Inverse of the 2x2 effective mass matrix (symmetric).
...
/// Rod: keeps the two anchors a fixed distance apart, the bodies may rotate freely about the anchors.
export class DistanceJoint extends Joint {
  ...
  prepare(invDt: number): void {
    ...
    this.effectiveMass = 1 / (a.invMass + b.invMass + a.invInertia * crossA * crossA + b.invInertia * crossB * crossB);
    this.bias = BIAS_FACTOR * invDt * (distance - this.length);
  }
  ...
  solve(): void {
    ...
    const lambda = -this.effectiveMass * (speed + this.bias);
    this.impulse += lambda;
    this.applyImpulse(this.ux * lambda, this.uy * lambda);
  }
}
